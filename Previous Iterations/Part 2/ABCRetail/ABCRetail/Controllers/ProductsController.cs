 ...
 */
        ...
        public async Task<IActionResult> ConfirmOrder(CartViewModel model)
        {
            ...
            // Send a message to the Azure Function
            await SendToFunctionAsync(message);

            return RedirectToAction("OrderConfirmation");
        }

        private async Task SendToFunctionAsync(string message)
        {
            HttpClient client = new HttpClient();
            var functionUrl = "https://ishkfunction.azurewebsites.net/api/QueueOrderMessage"; // Update URL
            var content = new StringContent(JsonSerializer.Serialize(new { Message = message }), Encoding.UTF8, "application/json");
            var response =  client.PostAsync(functionUrl, content);
